...
//  MAIN APP
// ═══════════════════════════════════════════════════════════════════
export default function ObsidianForge() {
  ...
  // Persistence
  useEffect(()=>{try{const r=localStorage.getItem("obsidian-forge-data");if(r){const p=JSON.parse(r);if(p.docs?.length)setDocs(p.docs);if(p.templates)setTemplates(p.templates);if(typeof p.activeDoc==="number")setActiveDoc(Math.min(p.activeDoc,(p.docs?.length||1)-1))}}catch{}},[]);
  const persist=useCallback((nd,nt,na)=>{try{localStorage.setItem("obsidian-forge-data",JSON.stringify({docs:nd??docs,templates:nt??templates,activeDoc:na??activeDoc}))}catch{}},[docs,templates,activeDoc]);
  useEffect(()=>{const t=setTimeout(()=>persist(),600);return()=>clearTimeout(t)},[docs,templates,activeDoc,persist]);
  useEffect(()=>{localStorage.setItem("obsidian-forge-theme",theme)},[theme]);
  ...
  // Templates
  const saveTpl=(n,d,c)=>{const nt=[...templates,{name:n,desc:d,category:c,content:docs[activeDoc].content,created:Date.now()}];setTemplates(nt);persist(undefined,nt);flash(`Template "${n}" saved`)};
  ...
  const delTpl=(i)=>{const nt=templates.filter((_,j)=>j!==i);setTemplates(nt);persist(undefined,nt)};
